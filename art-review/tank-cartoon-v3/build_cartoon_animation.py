...
SOURCE=OUT/'Cartoon_Asset_Source.blend'
...
spec=json.loads((OUT/'source_manifest.json').read_text())
records=[]
...
hot=mat('FX | hot yellow core',(1,.82,.15),1,.4)
orange=mat('FX | orange flame',(1,.22,.02),1,.25)
...
ember=mat('FX | ember',(1,.47,.04),1,.5)
...
rubber=bpy.data.materials['Cartoon / ink tracks']
treadmat=bpy.data.materials['Cartoon / tread tabs']
...
def scene(name,scale=10,loc=(8,-11,13),target=(0,-.4,0),res=(960,720)):
    ...
    SC.view_settings.view_transform='Standard';SC.unit_settings.system='METRIC'
...
for ix,source in enumerate(sources):
    ...
    trackw=.30 if w<1.4 else .36;a=l*.5-.22;perimeter=4*a+2*math.pi*.22
    linkcount=14;links=[]
    for side in [-1,1]:
        for j in range(linkcount):
            o=cube(f'Tread {side:+d} / {j:02d}',(0,0,0),(trackw+.012,.105,.045),treadmat,root)
            for f in list(range(1,110))+[112,239,240,325,326,330,336]:
                drive=min(max(f-1,0),71)*1.7/71
                turn=min(max(f-72,0),36)*math.radians(35)*(w/2-trackw/2)/36
            ...
            links.append(o)
    for o in wheels:
        base=o.rotation_euler.to_quaternion();side=-1 if o.location.x<0 else 1
        o.rotation_mode='QUATERNION'
        for f in list(range(1,110))+[112,239,240,325,326,330,336]:
            drive=min(max(f-1,0),71)*1.7/71
            yaw=min(max(f-72,0),36)*math.radians(35)/36
            travel=drive-side*yaw*(w/2-trackw/2)
            if f>=326:travel=0
            spin=Quaternion((1,0,0),travel/.17)
            key(o,'rotation_quaternion',f,spin@base)
...
for screen in bpy.data.screens:
    for area in screen.areas:
        if area.type=='VIEW_3D':
            ...
            area.spaces.active.overlay.show_overlays=False;area.spaces.active.shading.type='SOLID';area.spaces.active.shading.color_type='MATERIAL';area.spaces.active.shading.show_shadows=False
        if area.type=='DOPESHEET_EDITOR':
            area.spaces.active.dopesheet.show_only_selected=False
for sc in bpy.data.scenes:sc.frame_set(1)
bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'Pocket_Armor_Cartoon_Review.blend'))
