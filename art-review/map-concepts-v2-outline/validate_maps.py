...
bpy.ops.wm.open_mainfile(filepath=str(OUT/'Pocket_Armor_Map_Concepts_Outlined.blend'))
rows = []
...
for item in manifest['maps']:
    ...
    assert scene.render.use_freestyle
    lines=scene.view_layers[0].freestyle_settings.linesets[0]
    assert lines.select_by_collection and lines.collection is not None
    assert abs(lines.linestyle.thickness-1.4)<.001
...
map_source=OUT.parent/'map-concepts-v1/Pocket_Armor_Map_Concepts.blend'
assert hashlib.sha256(map_source.read_bytes()).hexdigest()==manifest['style_revision']['source_map_sha256']
